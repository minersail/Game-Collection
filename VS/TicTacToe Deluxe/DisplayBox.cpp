...
DisplayBox::DisplayBox(sf::Vector2i loc, float width, float height, std::string filename)
{
	Load(filename);
	assert(IsLoaded());
...
DisplayBox::DisplayBox(sf::Vector2i loc, float width, float height, std::string filename, std::string message)
{
	Load(filename);
	assert(IsLoaded());
	texturefile = filename;
