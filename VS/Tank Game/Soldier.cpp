...
Soldier::Soldier(std::string filename, float life, Faction fac)
{
	...
	GetSprite().setOrigin(GetTextureRect().width / 2, GetTextureRect().height / 6);
	...
	infantry = true;
	dispRect.height = (int)GetTextureRect().height / 3;
	dispRect.width = (int)GetTextureRect().width;
	GetSprite().setTextureRect(dispRect);
	LifeBar.SetScale(0.5);

	if (fac == Enemy)
	{
		TileData::CurrentTile->EnemyList.push_back(this); // To initialize soldiers created in TileData
		Game::EnemyList.push_back(this); // To initialize soldiers created during game loop
	}
	else if (fac == Ally)
	{
		TileData::CurrentTile->AllyList.push_back(this); // To initialize soldiers created in TileData
...
void Soldier::Update(float deltaTime, sf::Event ev)
{
	...
	if (!collided) // Normal behavior, pre-obstruction
	{
		...
		{
			if (sqrt(pow(abs(dx), 2) + pow(abs(dy), 2)) > 300) // If the nearest enemy is more than 300 pixels away chase it
			{
			...
			else // Fire instead
			{
				if (fmod(frameCount, 20) == 0) // Cast will be three times a second, adjustable if necessary
				{
					...
					RayCast cast(sf::Vector2i(16, 16), GetPosition(), GetRotation(false), 300.0f); // Cast out directly in front of the soldier the distance of its range
	...
	else // collided flag has been flipped and trace direction has been initialized; follow specific behavior for object tracing
	{
		...
		if (fmod(frameCount, 20) == 0) // Ray tracing is laggy... Perhaps a better solution can be found
		{
			...
			RayCast cast(sf::Vector2i(GetTextureRect().width, GetTextureRect().height), GetPosition(), traceDir * 3.141592f / 2, 30.0f);
			if (cast.Cast(hitEntity, Neutral))
			{
				if (fmod(frameCount, 20) == 0)
				{
...
{
	if (GetPosition().y < hitEntity.GetPosition().y - hitEntity.GetTextureRect().height / 2) // Hit top edge, traceDir will be down
	{
	...
	else if (GetPosition().y > hitEntity.GetPosition().y + hitEntity.GetTextureRect().height / 2) // Hit bottom edge, traceDir will be up
	{
		if (dx > 0) // Target is on right
			SetRotation(0);
		else // Target is on left
...
{
	Projectile* bullet = new Projectile("images/chris2.png", angle, 6, faction);
	Game::GetObjectQueue().Add("chris", bullet, "back");
